...
def sort_SR(costs: np.ndarray, cvs: np.ndarray, Pf: float = 0.45, iters: int = None) -> np.ndarray:
    """
    ...
    for _ in range(iters):
        ...
        for j in range(N - 1):
            ...
            use_objective = both_feasible or (random.random() < Pf)
            # tiêu chí so sánh
            if use_objective:
...
def initialize_mp(Individual_class,
    ...
    - Individual_class: callable tạo Individual (vd: lambda: Individual(np.zeros(dims)))
    - pop_size: kích thước quần thể cho mỗi task
    - Tasks: danh sách Task
    - dims: số chiều khởi tạo (chuẩn hoá [0,1])
    ...
    for t, task in enumerate(Tasks):
        # 1) Khởi tạo ngẫu nhiên rnvec trong [0,1]^dims
        pop_t = []
        for _ in range(pop_size):
            ind = Individual_class()
            # rnvec kích thước theo 'dims' được truyền vào (theo mã MATLAB)
            ind.rnvec = np.random.rand(dims)
            pop_t.append(ind)
        ...
        # 4) Lưu best theo rank(1)
        best_idx = int(rank[0])
        bestobj.append(pop_t[best_idx].factorial_costs)
        bestCV.append(pop_t[best_idx].constraint_violation)
        bestX.append(pop_t[best_idx].rnvec.copy())
...
if __name__ == "__main__":
    ...
    pop_size = 6
    dims = 10  # giống MATLAB: dùng một 'dims' để khởi tạo rnvec
    ...
    pop, calls, bestobj, bestCV, bestX = initialize_mp(
        Individual_class, pop_size, Tasks, dims,
        init_type='Stochastic_Ranking', var=var_sr
    )
    ...
    pop, calls, bestobj, bestCV, bestX = initialize_mp(
        Individual_class, pop_size, Tasks, dims,
        init_type='Epsilon_Constraint', var=var_ec
    )
